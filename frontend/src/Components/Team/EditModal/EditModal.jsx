...
import { Button, Header, Icon, Modal, Input, TextArea, Grid, Divider, Segment, Label } from 'semantic-ui-react';
import {
  addUserToTeam,
  deleteUserFromTeam,
  getUsersOfTeam,
      ...
      <Modal.Content>
        <Segment basic>
          <Grid columns={2} relaxed={'very'} stackable>
            ...
            <Grid.Column>
              <h3>Team name:</h3>
              <Input error={isNewNameEmpty} placeholder={'Team name'} value={newName} onChange={handleNewNameOnChange} />
            ...
            <Grid.Column >
              ...
              <Segment basic loading={isMembersLoading} >
                {members.map( user =>
                    <Button key={user.id} animated className={styles.userButton} onClick={()=> handleDeleteMember(user.id)}>
              ...
              <Segment basic loading={isUsersLoading}>
                {newUsers.map( user =>
                  <Button key={user.id} animated className={styles.userButton} onClick={() => handleAddMember(user.id)}>
                    <Button.Content visible>{user.username}</Button.Content>
          ...
          <Divider fitted vertical> <Icon name={'pencil'}/> </Divider>
        </Segment>
      </Modal.Content>
      <Modal.Actions>
